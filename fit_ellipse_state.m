function [focus1, focus2, long_axis] = fit_ellipse_state( x,y,axis_handle )
%
...
case (test>0),  status = '';
case (test==0), 
    status = 'Parabola found';  %warning( 'fit_ellipse: Did not locate an ellipse' );
case (test<0),
    status = 'Hyperbola found'; %warning( 'fit_ellipse: Did not locate an ellipse' );
end
...
if (nargin>2) & ~isempty( axis_handle ) & (test>0)
    ...
    %% shizuo
      if a>=b
        ...
        x_focus2 = focus2(1);
        y_focus2 = focus2(2);
        focus1 = [x_focus1 y_focus1];
        focus2 = [x_focus2 y_focus2];
    elseif a<b
        c = sqrt(b^2-a^2);
        focus1 = R * [X0;Y0 + c];
        focus2 = R * [X0;Y0 - c];
...
%     plot(yc, xc);
%     hold on
%     plot(y_focus1, x_focus1, '*')
%     hold on
%     plot(y_focus2, x_focus2, '*')
